  ...
  <div class="container address-list">
    ...
    <div class="card-footer">
      <div class="card-footer-left">
        <span v-show="selectedVoucher.totalAmount <= 0">请选择兑换卡，完成订单支付</span>

        <span
          class="card-un"
          v-show="selectedVoucher.totalAmount > 0 && selectedVoucher.totalAmount < orderPay.orderAmount"
        >已选择{{selectedVoucher.totalNum}}次，需支付：￥{{orderPay.orderAmount - selectedVoucher.totalAmount | priceFormat}}</span>

        <span class="on" v-show="selectedVoucher.totalAmount >= orderPay.orderAmount">足额支付</span>
      </div>
      <div
    ...
    <div class="card-modo" v-show="isAddVoucher" @click="isAddVoucher = false">
      <div class="card-modo-in" @click.stop="1">
        <h3 class="card-modo-name">添加兑换卡</h3>
...
export default {
  ...
  methods: {
    ...
    calculateTotalAmount() {
      ...
      vm.userVoucherList.forEach((e, i) => {
        if (e.selected) {
          amount += vm.$base.numMulti(e.currentNum, e.realValue);
          num += e.currentNum;
        }
      });
