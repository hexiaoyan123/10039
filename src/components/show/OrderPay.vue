<template>
  <div class="container order-pay">
    ...
    <div class="fix-bar">
      ...
      <div class="fix-bar-right" v-if=" toBePaid > 0 && payChannel.indexOf(8) >= 0 && (! $route.query.wechat)" v-show="timer != '00:00'" @click="goWechat">
        获取微信授权
      </div>
      <div class="fix-bar-right" v-if="(toBePaid == 0 || payChannel.indexOf(8) < 0 || $route.query.wechat)" v-show="timer != '00:00'" @click="payOk">
        确认支付
      </div>
    </div>
    <!-- 购物车控制结束 -->
...
export default {
  ...
  methods: {
    ...
    payOk () {
      ...
      this.$http.post('/panda-ticket-api/v1/paymentOrder', postData).then(function (response) {
    ...
    getListPayChannel () {
      let vm = this;
      this.$http.get('/panda-ticket-api/v1/listPayChannel').then(function (response) {
        if(response.data.code == '1000'){
